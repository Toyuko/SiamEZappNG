...
export default function ServiceDetailScreen() {
  ...
  const webBase = appConfig.webBaseUrl.replace(/\/+$/, '');

  const openContact = async () => {
    const url = `${webBase}/contact`;
  ...
  const openWebsiteService = async () => {
    const canonicalSlug = toBackendServiceSlug(service.slug);
    const url = `${webBase}/en/services/${canonicalSlug}`;
    const canOpen = await Linking.canOpenURL(url);
    if (!canOpen) {
      Alert.alert(t('serviceDetail.cannotOpenLink'), t('serviceDetail.tryAgainLater'));
      return;
